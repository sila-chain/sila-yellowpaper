...
The JavaScript API provides a consistent API across multiple scenarios including each of the clients' web-based in-process \DH{}App frameworks and the out-of-process RPC-based infrastructure. All key access takes places though the special \texttt{sil} object, part of the global namespace.
...
Values are implicitly converted between integers and hashes/byte-arrays; when this happens, integers are interpreted as big-endian as is standard for Sila. The following forms are allowed; they are all interpreted in the same way:
...
\subsection{The \texttt{sil} object}
...
\item \texttt{bCode}, the binary string (byte array) of SAVM-bytecode for the initialisation of the account;
