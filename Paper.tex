...
\usepackage{tikz}
\usepackage[bookmarks=true, unicode=true, pdftitle={Sila Yellow Paper: a formal specification of Sila, a programmable blockchain}, pdfauthor={Dr. Gavin Wood},pdfkeywords={Sila, Yellow Paper, blockchain, virtual machine, cryptography, decentralised, singleton, transaction, generalised},pdfborder={0 0 0.5 [1 3]}]{hyperref}
%,pagebackref=true
...
\title[Sila: A Secure Decentralised Generalised Transaction Ledger\\ \smaller \textbf{{SilaShanghai version}}]{Sila: A Secure Decentralised Generalised Transaction Ledger \\ \smaller \textbf{{SilaShanghai version \YellowPaperVersionNumber}}}
\author{
    Dr. Gavin Wood\\
    Founder, Sila \& Parity\\
    gavin@parity.io
...
Sila implements this paradigm in a generalised manner. Furthermore it provides a plurality of such resources, each with a distinct state and operating code but able to interact through a message-passing framework with others. We discuss its design, implementation issues, the opportunities it provides and the future hurdles we envisage.
\end{abstract}
...
\cite{buterin2013sila} first proposed the kernel of this work in late November, 2013. Though now evolved in many ways, the key functionality of a block-chain with a Turing-complete language and an effectively unlimited inter-transaction storage capability remains unchanged.
...
Sila, taken as a whole, can be viewed as a transaction-based state machine: we begin with a genesis state and incrementally execute transactions to morph it into some current state. It is this current state which we accept as the canonical ``version'' of the world of Sila. The state can include such information as account balances, reputations, trust arrangements, data pertaining to information of the physical world; in short, anything that can currently be represented by a computer is admissible. Transactions thus represent a valid arc between two states; the `valid' part is important---there exist far more invalid state changes than valid state changes. Invalid state changes might, \eg, be things such as reducing an account balance without an equal and opposite increase elsewhere. A valid state transition is one which comes about through a transaction. Formally:
\begin{equation}
\linkdest{Upsilon_state_transition}\linkdest{Upsilon}\boldsymbol{\sigma}_{t+1} \equiv \Upsilon(\boldsymbol{\sigma}_{t}, T)
\end{equation}

where $\Upsilon$ is the Sila state transition function. In Sila, $\Upsilon$, together with $\boldsymbol{\sigma}$ are considerably more powerful than any existing comparable system; $\Upsilon$ allows components to carry out arbitrary computation, while $\boldsymbol{\sigma}$ allows components to store arbitrary state between transactions.
...
Where \hyperlink{block}{$B$} is this block, which includes a series of transactions amongst some other components and $\hyperlink{Pi}{\Pi}$ is the block-level state-transition function for transactions\footnote{Note that since the SilaShanghai fork, blocks also needs to process \textit{withdrawal operations} in order to reach their final state. Withdrawal operations are defined in sub-section \ref{subsec:The_Withdrawal}, and block final state is discussed in greater detail in section \ref{ch:finalisation}.}.

This is the basis of the blockchain paradigm, a model that forms the backbone of not only Sila, but all decentralised consensus-based transaction systems to date.

\subsection{Value}

In order to incentivise computation within the network, there needs to be an agreed method for transmitting value. To address this issue, Sila has an intrinsic currency, Sila, known also as {\small SIL} and sometimes referred to by the Old English \DH{}. The smallest subdenomination of Sila, and thus the one in which all integer values of the currency are counted, is the Wei. One Sila is defined as being $10^{18}$ Wei. There exist other subdenominations of Sila:
\par
\begin{center}
\begin{tabular}{rl}
\toprule
...
Throughout the present work, any reference to value, in the context of Sila, currency, a balance or a payment, should be assumed to be counted in Wei.
...
Since the \textit{SilaParis} hard fork, reaching consensus on new blocks is managed by a protocol called the \textit{Beacon Chain}.
It is known as the \textit{consensus layer} of Sila, and it defines the rules for determining the canonical history of Sila blocks.
This document describes the \textit{execution layer} of Sila.
The execution layer defines the rules for interacting with and updating the state of the Sila virtual machine.
The consensus layer is described in greater detail in the \href{https://github.com/sila-chain/consensus-specs}{consensus specifications}.
How the consensus layer is used to determine the canonical state of Sila is discussed in section \ref{ch:blocktree_to_blockchain}.

There are many versions of Sila, as the protocol has undergone a number of updates.
These updates can be specified to occur:
\begin{itemize}
  \item at a particular block number in the case of pre-\textit{SilaParis} updates,
  \item after reaching a \textit{terminal total difficulty} in the case of the \textit{SilaParis} update, or
  \item at a particular block timestamp in the case of post-\textit{SilaParis} updates.
\end{itemize}
This document describes the \textit{SilaShanghai} version.

In order to follow back the history of a path, one must reference multiple versions of this document.
Here are the block numbers of protocol updates on the Sila main network:\footnote{Note that while the SilaParis, SilaShanghai, and every upcoming forks activated at a given block number (e.g. 15,537,394 for SilaParis), the trigger was not the block number, but rather reaching a specified \textit{timestamp} (or \textit{total difficulty} for SilaParis). The trigger for the \textit{SilaParis} and subsequent hard fork are discussed in greater detail in section \ref{ch:pos_transition}.}
\par
\begin{center}
\begin{tabular}{lr}
\toprule
...
$F_{\mathrm{SilaParis}}$             & 15537394 \\
$F_{\mathrm{SilaShanghai}}$          & 17034870 \\
\bottomrule
\end{tabular}
\end{center}
\par
...
In order to distinguish between diverged blockchains, SIP-155 by \cite{SIP-155} introduced the concept of chain ID, which we denote by $\beta$.
For the Sila main network
\begin{equation}
  \linkdest{chain_id}
  \beta = 1
\end{equation}
...
Functions operating on highly structured values are denoted with an upper-case Greek letter, \eg \hyperlink{Upsilon_state_transition}{$\Upsilon$}, the Sila state transition function.
...
Having introduced the basic concepts behind Sila, we will discuss the meaning of a transaction, a block and the state in more detail.
...
A transaction (formally, $T$) is a single cryptographically-signed instruction constructed by an actor externally to the scope of Sila.
...
SIP-2718 by \cite{SIP-2718} introduced the notion of different transaction types.
As of the \textit{London} version of the protocol, there are three transaction types: 0 (legacy), 1 (SIP-2930 by \cite{SIP-2930}), and 2 (SIP-1559 by \cite{SIP-1559}).
...
\begin{description}
\item[type]\linkdest{tx_type}{} SIP-2718 transaction type; formally $T_{\mathrm{x}}$.
...
SIP-2930 (type 1) and SIP-1559 (type 2) transactions also have:
\begin{description}
...
$T_{\mathrm{w}} = 27 + T_{\mathrm{y}}$ or $T_{\mathrm{w}} = 2\beta + 35 + T_{\mathrm{y}}$ (see SIP-155 by \cite{SIP-155}).
\end{description}

There are differences in how one's acceptable gas price is specified in type 2 transactions versus type 0 and type 1 transactions. Type 2 transactions take better advantage of the gas market improvements introduced in SIP-1559 by explicitly limiting the \textit{priority fee}\footnote{The \textit{priority fee} is discussed in greater detail in sections \ref{ch:payment} and \ref{ch:transactions}.} that is paid. Type 2 transactions have the following two fields related to gas:
...
A withdrawal (formally, \textit{W}) is a tuple of data describing a consensus layer validator's withdrawal of some amount of its staked Sila.
...
\item[recipient] the 20-byte address that will receives Sila from this withdrawal; formally $W_{\mathrm{r}}$.
\item[amount] a nonzero amount of Sila denominated in Gwei ($10^9$ Wei); formally $W_{\mathrm{a}}$.
\end{description}
...
The block in Sila is the collection of relevant pieces of information (known as the block \textit{header}), $H$, together with information corresponding to the comprised transactions,
$\mathbf{T}$,\hypertarget{ommerheaders}{} a now deprecated property $\mathbf{U}$ that prior to the \textit{SilaParis} hard fork contained headers of blocks whose parents were equal to the present block's parent's parent (such blocks were known as \textit{ommers}\footnote{\textit{ommer} is a gender-neutral term to mean ``sibling of parent''; see \url{https://nonbinary.miraheze.org/wiki/Gender_neutral_language_in_English\#Aunt/Uncle}}),
and, since the \textit{SilaShanghai} hard fork, $\mathbf{W}$, a collection of validator's withdrawal pushed by the consensus layer. The block header contains several pieces of information:
...
\item[prevRandao]\linkdest{prevRandao_H__a}{}\linkdest{H__a} the latest RANDAO mix\footnote{RANDAO is a pseudorandom value generated by validators on the Sila consensus layer. Refer to the consensus layer specs (\url{https://github.com/sila-chain/consensus-specs}) for more detail on RANDAO.} of the post beacon state of the previous block; formally $H_{\mathrm{a}}$.
...
similarly, $p_{\mathrm{T}}(k, v)$ and $p_{\mathrm{R}}(k, v)$ are pairwise RLP transformations, but with a special treatment for SIP-2718 transactions:
\begin{equation}
...
\end{eqnarray}
where $\widetilde{L}_{\mathrm{T}}$ takes a special care of SIP-2718 transactions:
\begin{equation}
...
\linkdest{H__f}The \textit{London} release introduced the block attribute \textit{base fee per gas} \hyperlink{block_baseFeePerGas_H__f}{$H_{\mathrm{f}}$} (see SIP-1559 by \cite{SIP-1559}).
...
The \textit{SilaParis} hard fork changed Sila's consensus from proof of work to proof of stake, and thus deprecated many block header properties related to proof of work. These deprecated properties include \textbf{nonce} ($H_{\mathbf{n}}$), \textbf{ommersHash} ($H_{\mathbf{o}}$), \textbf{difficulty} ($H_{\mathbf{d}}$), and \textbf{mixHash} ($H_{\mathbf{m}}$).
...
Transactors are free to select any priority fee that they wish, however validators are free to ignore transactions as they choose. A higher priority fee on a transaction will therefore cost the sender more in terms of Sila and deliver a greater value to the validator and thus will more likely be selected for inclusion. Since there will be a (weighted) distribution of minimum acceptable priority fees, transactors will necessarily have a trade-off to make between lowering the priority fee and maximising the chance that their transaction will be included in a block in a timely manner.

%\subsubsection{Determining Computation Costs}

\section{Transaction Execution} \label{ch:transactions}

The execution of a transaction is the most complex part of the Sila protocol: it defines the state transition function \hyperlink{Upsilon_state_transition}{$\Upsilon$}. It is assumed that any transactions executed first pass the initial tests of intrinsic validity. These include:
...
\item the sender account has no contract code deployed (see SIP-3607 by \cite{SIP-3607});
...
\hypertarget{tx_log_series_wordy_defn_A__l}{} $A_{\mathbf{l}}$ is the log series: this is a series of archived and indexable `checkpoints' in VM code execution that allow for contract-calls to be easily tracked by onlookers external to the Sila world (such as decentralised application front-ends).
...
Finally, SIP-2929 by \cite{SIP-2929} introduced \hypertarget{accessed_addresses_defn_words_A__a}{}$A_{\mathbf{a}}$, the set of accessed account addresses, and \hypertarget{accessed_storage_keys_defn_words_A__k}{}$A_{\mathbf{K}}$, the set of accessed storage keys
...
\end{align}
where $T_{\mathbf{i}},T_{\mathbf{d}}$ means the series of bytes of the transaction's associated data and initialisation SAVM-code, depending on whether the transaction is for contract-creation or message-call.
...
There are a number of intrinsic parameters used when creating an account: sender ($s$), original transactor\footnote{which can differ from the sender in the case of a message call or contract creation not directly triggered by a transaction but coming from the execution of SAVM-code} ($o$),
available gas ($g$), effective gas price ($p$), endowment ($v$) together with an arbitrary length byte array, $\mathbf{i}$, the initialisation SAVM code, the present depth of the message-call/contract-creation stack ($e$), the salt for new account's address ($\zeta$) and finally the permission to make modifications to the state ($w$).
...
For {\small \hyperlink{create2}{CREATE2}} the rule is different and is described in SIP-1014 by \cite{SIP-1014}.
...
Finally, the account is initialised through the execution of the initialising SAVM code $\mathbf{i}$ according to the execution model (see section \ref{ch:model}).
...
Note the last condition of $F$ indicates that contract code cannot begin with the byte \texttt{0xef} (refer to SIP-3541 by \cite{SIP-3541}),
...
See Appendix \ref{app:vm} for a rigorous definition of the SAVM gas cost.
...
-- see SIP-2200 by \cite{SIP-2200} for more information.
...
The \textit{SilaParis} hard fork changed the underlying consensus mechanism of Sila from proof of work to proof of stake.

Unlike all previous hard forks of Sila, \textit{SilaParis} was not defined to occur at a particular block height, but rather after a specified \textit{terminal total difficulty} was reached.
...
Thus the \textit{terminal block}, the last proof of work block before the \textit{SilaParis} fork takes effect, is defined as having:
\begin{eqnarray}
...
Total difficulty for a proof of work (pre-\textit{SilaParis}) block $B$ is defined recursively as:
\begin{eqnarray}
...
\subsection{Post-SilaParis Updates}
Because the Beacon Chain generate a new slot every 12 seconds, post-\textit{SilaParis} updates can be scheduled to occur at a specific timestamp.
At the execution layer, the update will then happen in the first produced block after the scheduled timestamp.
For example the \textit{SilaShanghai} hard fork was scheduled to occur at 2023-04-12 10:27:35 UTC, on Epoch 194,048.
Validators failed to propose a block during the two first slots of this Epoch, but at slot 6,209,538 a validator finally proposed the block 17,034,870, marking the transition to \textit{SilaShanghai} on the execution layer.

\section{Blocktree to Blockchain} \label{ch:blocktree_to_blockchain}

Prior to the transition to proof of stake at the \textit{SilaParis} hard fork, the canonical blockchain was defined as the block path with the greatest \textit{total difficulty}, defined in section \ref{ch:pos_transition} as $B_{\mathrm{t}}$.
...
Note that in order to determine what blocks comprise the canonical Sila blockchain after \textit{SilaParis}, one must have additional information from the Beacon Chain, which is not described herein. We denote events emitted by the Beacon Chain with the prefix $\mathtt{POS\_}$.
...
A data feed contract is one which provides a single service: it gives access to information from the external world within Sila. The accuracy and timeliness of this information is not guaranteed and it is the task of a secondary contract author---the contract that utilises the data feed---to determine how much trust can be placed in any single data feed.

The general pattern involves a single contract within Sila which, when given a message call, replies with some timely information concerning an external phenomenon. An example might be the local temperature of New York City. This would be implemented as a contract that returned that value of some known point in storage. Of course this point in storage must be maintained with the correct such temperature, and thus the second part of the pattern would be for an external server to run an Sila node, and immediately on discovery of a new block, creates a new valid transaction, sent to the contract, updating said value in storage. The contract's code would accept such updates only from the identity contained on said server.
...
Finally, blockchain compression could perhaps be conducted: nodes in state trie that haven't sent/received a transaction in some constant amount of blocks could be thrown out, reducing both Sila-leakage and the growth of the state database.

\section{Conclusion} \label{ch:conclusion}

We have introduced, discussed and formally defined the protocol of Sila. Through this protocol the reader may implement a node on the Sila network and join others in a decentralised secure social operating system. Contracts may be authored in order to algorithmically specify and autonomously enforce rules of interaction.

\section{Acknowledgements}

Many thanks to Aeron Buchanan for authoring the \textit{Homestead} revisions, Christoph Jentzsch for authoring the Ethash algorithm and Yoichi Hirai for doing most of the SIP-150 changes. Important maintenance, useful corrections and suggestions were provided by a number of others from the Sila DEV organisation and Sila community at large including Gustav Simonsson, Pawe\l{} Bylica, Jutta Steiner, Nick Savers, Viktor Tr\'{o}n, Marko Simovic, Giacomo Tazzari and, of course, Vitalik Buterin.

\section{Availability}

The source of this paper is maintained at \url{https://github.com/sila-chain/sila-yellowpaper/}. A published PDF is not currently configured for this repository; use \texttt{build.sh} to generate it locally.
...
\begin{description}
\item[External Actor] A person or other entity able to interface to an Sila node, but external to the world of Sila. It can interact with Sila through depositing signed Transactions and inspecting the blockchain and associated state. Has one (or more) intrinsic Accounts.

\item[Address] A 160-bit code used for identifying Accounts.

\item[Account] Accounts have an intrinsic balance and transaction count maintained as part of the Sila state. They also have some (possibly empty) SAVM Code and a (possibly empty) Storage State associated with them. Though homogenous, it makes sense to distinguish between two practical types of account: those with empty associated SAVM Code (thus the account balance is controlled, if at all, by some external entity) and those with non-empty associated SAVM Code (thus the account represents an Autonomous Object). Each Account has a single Address that identifies it.
...
\item[Autonomous Object] A notional object existent only within the hypothetical state of Sila. Has an intrinsic address and thus an associated account; the account will have non-empty associated SAVM Code. Incorporated only as the Storage State of that account.

\item[Storage State] The information particular to a given Account that is maintained between the times that the Account's associated SAVM Code runs.

\item[Message] Data (as a set of bytes) and Value (specified as Sila) that is passed between two Accounts, either through the deterministic operation of an Autonomous Object or the cryptographically secure signature of the Transaction.

\item[Message Call] The act of passing a message from one Account to another. If the destination account is associated with non-empty SAVM Code, then the VM will be started with the state of said Object and the Message acted upon. If the message sender is an Autonomous Object, then the Call passes any data returned from the VM operation.

\item[Gas] The fundamental network cost unit. Paid for exclusively by Sila (as of PoC-4), which is converted freely to and from Gas as required. Gas does not exist outside of the internal Sila computation engine; its price is set by the Transaction and validators are free to ignore Transactions whose Gas price is too low.

\item[Contract] Informal term used to mean both a piece of SAVM Code that may be associated with an Account or an Autonomous Object.

\item[Object] Synonym for Autonomous Object.

\item[App] An end-user-visible application hosted in the Sila Browser.

\item[Sila Browser] (aka Sila Reference Client) A cross-platform GUI of an interface similar to a simplified browser (a la Chrome) that is able to host sandboxed applications whose backend is purely on the Sila protocol.

\item[Sila Virtual Machine] (aka SAVM) The virtual machine that forms the key part of the execution model for an Account's associated SAVM Code.

\item[Sila Runtime Environment] (aka ERE) The environment which is provided to an Autonomous Object executing in the SAVM. Includes the SAVM but also the structure of the world state on which the SAVM relies for certain I/O instructions including CALL \& CREATE.

\item[SAVM Code] The bytecode that the SAVM can natively execute. Used to formally specify the meaning and ramifications of a message to an Account.

\item[SAVM Assembly] The human-readable form of SAVM-code.
...
SIP-152 by \cite{SIP-152} defines $\Xi_{\mathtt{BLAKE2\_F}}$ as a precompiled contract implementing the compression function $\mathtt{F}$ used in the BLAKE2 cryptographic hashing algorithm.
...
\end{align}
Note that this restriction on $s$ is more stringent than restriction \ref{eq:ecrec_precompile_validity} in the $\Xi_{\mathtt{ECREC}}$ precompile; see SIP-2 by \cite{SIP-2} for more detail.

For a given private key, $p_{\mathrm{r}}$, the Sila address $A(p_{\mathrm{r}})$ (a 160-bit value) to which it corresponds is defined as the rightmost 160-bits of the Keccak-256 hash of the corresponding ECDSA public key:
\begin{equation}
...
As previously specified in section \ref{ch:model}, these definitions take place in the final context there. In particular we assume $O$ is the SAVM state-progression function and define the terms pertaining to the next cycle's state $(\boldsymbol{\sigma}', \boldsymbol{\mu}')$ such that:
\begin{equation}
...
&&&& $A'_{\mathbf{K}} \equiv A_{\mathbf{K}} \cup \{(I_{\mathrm{a}}, \boldsymbol{\mu}_{\mathbf{s}}[0])\}$ \\
&&&&\linkdest{C__SSTORE}{}$C_{\text{\tiny SSTORE}}(\boldsymbol{\sigma}, \boldsymbol{\mu})$ and \linkdest{A r}{}$A'_{\mathrm{r}}$ are specified by SIP-2200 as follows. \\
...
&&&& since the \textit{SilaShanghai} update, this opcode has been marked as deprecated. \\
...
This section is kept for historical purposes, but the Ethash algorithm is no longer used for consensus as of the \textit{SilaParis} hard fork.
